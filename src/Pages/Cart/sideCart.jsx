    ...
    return(
        <aside className={`${styles.sideCart} ${isOpen ? styles.open : ''}`}>
                    ...
                    <ul className={styles.cartList}>
                        {cartItems.map((item) => (
                            <li key={item.id} className={styles.cartItem}>
                                ...
                                <div>
                                    <p>{item.nome}</p> - R$ {Number(item.preco).toFixed(2)}
            ...
            <div className= {styles.DivisionButton} >
                <button className= {styles.ButtonCart} onClick={()=> navigation ('/cart')}>Carrinho <FaShoppingCart className={styles.IconButtonCart}/>
                </button>
